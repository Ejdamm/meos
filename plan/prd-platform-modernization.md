...
**Description:** As a developer, I want cross-platform blockers fixed in the legacy codebase *before* migration so that fewer problems arise during the migration process.

> **Rationale:** Case-sensitivity issues, Win32-specific API calls in domain logic, and tight GUI coupling cause friction when migrating files to `src/`. Fixing these in `code/` (under the existing MSBuild build) eliminates entire categories of migration errors. All changes must pass the existing Windows build.
...
- [ ] Windows build (MSBuild) still succeeds
