...
const wstring &oEvent::formatRogainingStringAux(const oPrintPost &pp, const oListParam &par, const RogainingLegInfo *rgLeg) const {
  ...
  if (pp.type != lString && (wsptr == 0 || wsptr->empty()) && bfw[0] == 0)
    ...
    if (pp.text.empty()) {
      ...
      wchar_t bf2[512];
      swprintf(bf2, pp.text.c_str(), bfw);
      wstring &res = StringCache::getInstance().wget();
      res = bf2;
      return res;
    }
...
const wstring &oEvent::formatListStringAux(const oPrintPost &pp, const oListParam &par,
  ...
  if (type != lString && (wsptr == 0 || wsptr->empty()) && wbf[0] == 0)
    ...
    if (pp.text.empty())
      ...
      swprintf(wbf, pp.text.c_str() + textOffset, wsptr->c_str());
      wstring &res = StringCache::getInstance().wget();
      res = wbf;
      return res;
    }
    ...
    if (pp.text.empty()) {
      ...
      wchar_t bf2[512];
      swprintf(bf2, pp.text.c_str() + textOffset, wbf);
      wstring &res = StringCache::getInstance().wget();
      res = bf2;
      return res;
    }
  ...
  switch (lt) {
    ...
    case unused_EStdTeamResultListLeg: {
      ...
      if (li.lp.getLegNumberCoded() != 1000)
        swprintf(title, 256, (lang.tl(L"Resultat efter sträcka X#" + li.lp.getLegName())+L" - %%s").c_str());
      else
        swprintf(title, 256, (lang.tl(L"Resultat efter sträckan")+L" - %%s").c_str());
    ...
    case unused_EStdTeamResultListLegLARGE: {
      wchar_t title[256];
      if (li.lp.getLegNumberCoded() != 1000)
        swprintf(title, 256, (L"%%s - "+lang.tl(L"sträcka X#" + li.lp.getLegName())).c_str());
      else
        swprintf(title, 256, (L"%%s - "+lang.tl(L"slutsträckan")).c_str());
    ...
    case EStdIndMultiStartListLeg:
      ...
      //snprintf(title, lang.tl("Startlista lopp %d - %%s").c_str(), li.lp.legNumber+1);
      ln=li.lp.getLegInfo(sampleClass);
