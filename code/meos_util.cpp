...
void processGeneralTime(const wstring &generalTime, wstring &meosTime, wstring &meosDate) {
  ...
  if (second >= 0 && minute >= 0 && hour>= 0) {
    if (!pm)
      ...
      int rawHour = std::stoi(parts[hour]);
      if (rawHour < 12)
        rawHour+=12;
  ...
  if (year >= 0 && month >= 0 && day >= 0) {
    int y = -1, m = -1, d = -1;
    if (year != month) {
      y = std::stoi(parts[year]);
      m = std::stoi(parts[month]);
      d = std::stoi(parts[day]);

      //meosDate = parts[year] + "-" + parts[month] + "-" + parts[day];
    } 
    else {
      int td = std::stoi(parts[year]);
      int y1 = td / 10000;
      int m1 = (td / 100) % 100;
      int d1 = td % 100;
