                          ...
                          int cardNo, const wstring &birthDate, bool autoAdd)
{
  int birthYear = 0;
  if (!birthDate.empty()) {
    int numY = (int)std::wcstol(birthDate.c_str(), nullptr, 10);
...
int oEvent::convertScore(const wstring &score) const {
  ...
  switch (scoreFactor.get()) {
  case 0:
    return (int)std::wcstol(score.c_str(), nullptr, 10);
  case 1:
    factor = 10;
    break;
  case 2:
...
int oEvent::convertAbsoluteTime(const wstring &m)
{
  ...
  if (timeConstSecond > 1 && firstComma > 0) {
    int sub = std::abs((int)std::wcstol(m.c_str() + firstComma + 1, nullptr, 10));
    while (sub >= timeConstSecond)
      sub /= timeConstSecond;
    t += sub;
  }
...
int oEvent::getRelativeTime(const wstring &m) const {
  ...
  if (dayIndex == 0)
    ...
    days = (int)std::wcstol(m.c_str(), nullptr, 10);
  }
  if (atime>=0 && atime <= timeConstHour*24){
    int rtime = atime-ZeroTime;
    ...
    public: 
      ...
      void handle(gdioutput& gdi, BaseInfo& info, GuiEventType type) final {
        if (type == GuiEventType::GUI_LISTBOX) {
          ...
          if (oe.baseButtonsCallback)
            oe.baseButtonsCallback(gdi, 1, false);
          gdi.refreshFast();
...
void oEvent::clear()
{
  ...
  if (tabAutoKillMachinesCallback)
    tabAutoKillMachinesCallback();
...
int oEvent::getPropertyInt(const char *name, int def)
{
  if (eventProperties.count(name)==1)
    return (int)std::wcstol(eventProperties[name].c_str(), nullptr, 10);
  else {
...
int oEvent::interpretCurrency(const wstring &c) const {
  if (tCurrencyFactor == 1 && tCurrencyPreSymbol == false)
    return (int)std::wcstol(c.c_str(), nullptr, 10);
...
int oEvent::interpretCurrency(double val, const wstring &cur)  {
  if (compareStringIgnoreCase(L"sek", cur.c_str()) == 0)
    setCurrency(1, L"kr", L",", false);
  else if (compareStringIgnoreCase(L"eur", cur.c_str()) == 0)
...
void oEvent::getExtraLines(const char *attrib, vector< pair<wstring, int> > &lines) const {
  ...
  for (size_t k = 0; k + 1 < splt.size(); k+=2) {
    lines.push_back(make_pair(splt[k], (int)std::wcstol(splt[k+1].c_str(), nullptr, 10)));
  }
...
void oEvent::supportSubSeconds(bool use) {
  if (setSubSecondModeCallback)
    setSubSecondModeCallback(use);
...
map<oEvent::ExtraFields, wstring> oEvent::getExtraFields(oEvent::ExtraFieldContext context) const {
  ...
  for (const wstring& w : sp) {
    int code = (int)std::wcstol(w.c_str(), nullptr, 10);
...
map<int, oPunch::SpecialPunch> oEvent::getPunchMapping() const {
  ...
  size_t pos = 0;
  while (pos < cMap.size()) {
    int code = (int)std::wcstol(cMap.data() + pos, nullptr, 10);
    pos = cMap.find_first_of('-', pos);
    if (code <= 0 || pos == wstring::npos)
      break;
    pos++;
    int value = (int)std::wcstol(cMap.data() + pos, nullptr, 10);
