...
int oEvent::convertAbsoluteTime(const wstring &m)
{
  ...
  if (kp>0)
  {
    ...
    if (kp>0) {
      second=std::stoi(mtext.substr(kp+1));
  ...
  if (timeConstSecond > 1 && firstComma > 0) {
    int sub = std::abs(std::stoi(m.c_str() + firstComma + 1));
    while (sub >= timeConstSecond)
      sub /= timeConstSecond;
    t += sub;
  }
    ...
    public: 
      ...
      void handle(gdioutput& gdi, BaseInfo& info, GuiEventType type) final {
        if (type == GuiEventType::GUI_LISTBOX) {
          ...
          if (oe.baseButtonsCallback) oe.baseButtonsCallback(gdi, 1, false);
          gdi.refreshFast();
        }
      }
    };
...
void oEvent::clear()
{
  ...
  if (tabAutoKillMachinesCallback) tabAutoKillMachinesCallback();
...
int oEvent::getPropertyInt(const char *name, int def)
{
  if (eventProperties.count(name)==1)
    return std::stoi(eventProperties[name]);
  else {
...
void oEvent::getExtraLines(const char *attrib, vector< pair<wstring, int> > &lines) const {
  ...
  for (size_t k = 0; k + 1 < splt.size(); k+=2) {
    lines.push_back(make_pair(splt[k], std::stoi(splt[k+1])));
  }
...
void oEvent::supportSubSeconds(bool use) {
  if (setSubSecondModeCallback) setSubSecondModeCallback(use);
...
map<int, oPunch::SpecialPunch> oEvent::getPunchMapping() const {
  ...
  size_t pos = 0;
  while (pos < cMap.size()) {
    int code = std::stoi(cMap.data() + pos);
    pos = cMap.find_first_of('-', pos);
    if (code <= 0 || pos == wstring::npos)
      break;
    pos++;
    int value = std::stoi(cMap.data() + pos);
