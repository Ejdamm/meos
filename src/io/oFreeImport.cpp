...
{
  int k=(int)std::wcstol(p, nullptr, 10);
                              ...
                              oEntryBlock &entry, vector<oEntryBlock> &entries, bool allowName)
{
  ...
  for (size_t k=0;k<words.size();) {
    if (isTimeV[k]) {
      ...
      entry.setCardNo((int)std::wcstol(words[k], nullptr, 10));
                               ...
                               vector<int> &classified)
{
  ...
  for (size_t j=0;j<p.size();j++) {
    ...
    if (type==-1 && ptrn.size()>1) {
      if (ptrn[j].isClub())
        ...
        type=1;
      else if (ptrn[j].isCard() && (int)std::wcstol(p[j], nullptr, 10)>0)
        type=3;
    }
  ...
  while (cards > names && names>0) {
    ...
    for (size_t k = 0; k<classified.size(); k++) {
      if (classified[k] == 3) {
        int a = idx>=0 ? (int)std::wcstol(p[idx], nullptr, 10) : -1;
        if (a==-1)
          idx = k;
        else {
          int b = (int)std::wcstol(p[k], nullptr, 10);
          if (b>0 && b<a)
            idx = k;
        }
      }
  ...
  for (size_t k=0;k<parts.size();k++) {
    ...
    for (int j=0;j<s;j++) {
      ...
      if (type == Unknown)
        ...
        lastInsertedType = Card;
        entry.setCardNo((int)std::wcstol(p[j], nullptr, 10));
        if (readNames&1) {
          readNames++;
          entry.completeName();
        }
