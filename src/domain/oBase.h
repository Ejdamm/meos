...
public:
  ...
  void setExtIdentifier(long long id);

  /// Get an external identifier (or 0) if none
  long long getExtIdentifier() const;
  ...
  static int idFromExtId(long long extId);
  static void converExtIdentifierString(long long raw, wchar_t bf[16]);
  static long long converExtIdentifierString(const wstring &str);
