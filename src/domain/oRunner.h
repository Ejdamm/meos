...
public:
  ...
  virtual const wstring &getClub() const {if (Club) return Club->getName(); else return _EmptyWString;}
...
public:
  ...
  static constexpr int encodeNameFormat(NameFormat f) {
    return int(f);
  }
  
  static constexpr NameFormat decodeNameFormat(int f) {
    return NameFormat(f);
  }
  ...
  cTeam getTeam() const {return (cTeam)tInTeam;}
