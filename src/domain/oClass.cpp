...
void oClass::updatedIgnoreStartPunch() {
  ...
  for (pRunner r : rr) {
    if (ignoreSP && r->getStartTime() > 0) {
      if (r->getCard()) {
        int st = r->getCard()->getStartTime(SpecialPunch::PunchStart);
        ...
        for (pFreePunch p : fp) {
          if (p->getTypeCode() == SpecialPunch::PunchStart && r->getStartTime() == p->getTimeInt()) {
            r->restoreDefaultStartTime(false);
            r->synchronize();
            updated = true;
          }
      ...
      int stCd = crs && crs->useFirstAsStart() && crs->getControl(0) ? crs->getControl(0)->getFirstNumber() : SpecialPunch::PunchStart;
      oe->getPunchesForRunner(r->getId(), false, fp);
      for (pFreePunch p : fp) {
...
{ /*
  char bf[256];
  snprintf(bf, "Insert to %d, %d, time %d\n", courseId, controlNo, time);
...
{/*
  char bf[256];
  snprintf(bf, "Get from %d,  %d, time %d\n", courseId, controlNo, time);
  OutputDebugString(bf);
  */
  if (tLegAccTimeToPlace) {
    int key = time + (controlNo + courseId*128)*16013;
