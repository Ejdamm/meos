...
struct oListParam {
  ...
  int timePerPage;
  int margin;
  int screenMode;// 0 normal window, 1 = page by page, 2 = screen
...
public:
  ...
  int getMaxCharWidth(oEvent &oe, 
                      ...
                      bool large = false, 
                      int minSize = 0) const {
    // This overload is just a stub to avoid compilation error in some places
    return 0;
  }
  ...
  oDataContainer &getDataBuffers(pvoid &data, pvoid &olddata, pvectorstr &strData) const override { static oDataContainer dummy(0); return dummy; }
  int getDISize() const override { return 0; }
  void merge(const oBase &input, const oBase *base) override {}
  wstring getInfo() const override { return Name; }
