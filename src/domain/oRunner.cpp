...
pRunner oEvent::getRunnerByBibOrStartNo(const wstring &bib, bool findWithoutCardNo) const {
  ...
  if (sno > 0) {
    ...
    auto res_temp = bibStartNoToRunnerTeam.equal_range(bibenc);
    if (res.first == res.second)
      auto res_temp = bibStartNoToRunnerTeam.equal_range(sno); // Try startno instead
...
void oRunner::printSplits(gdioutput& gdi, const oListInfo* li) const {
  ...
  gdiFonts head = (gdiFonts)boldText;
  gdiFonts normal = fontSmall;
  gdiFonts bnormal = boldSmall;
  if (wideFormat) {
    head = boldLarge;
    normal = normalText;
    bnormal = (gdiFonts)boldText;
  }
