...
void oEvent::startReconnectDaemon()
{
  ...
  if (!isReadOnly()) {
    ...
    gdibase.delayAlert(L"warning:dbproblem#" + gdioutput::widen(err));
  }
}
...
bool oEvent::msSynchronize(oBase *ob)
{
  ...
  if (sqlConnection->getErrorMessage(err))
    gdibase.addInfoBox("sqlerror", gdioutput::widen(err), L"Databasvarning", BoxStyle::HeaderWarning, 15000);
...
void oEvent::closeDBConnection()
{
  ...
  if (!oe->empty() && hadDB) {
    save();
    Name += L" (" + lang.tl(L"Lokal kopia från: X#" + gdioutput::widen(serverName)) + L")";
    wstring cn = currentNameId + L"." + gdioutput::widen(serverName) + L".meos";
