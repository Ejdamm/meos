...
bool oEvent::isCardRead(const void *card) const
{
...
int oCard::getStartPunchCode() const {
  ...
  if (it != punches.end()) {
    if (it->getTypeCode() == SpecialPunch::PunchStart)
...
int oCard::getStartTime(int ptype) const {
  auto it = punches.begin();
  if (ptype == SpecialPunch::PunchStart) {
    if (it != punches.end()) {
      if (it->getTypeCode() == SpecialPunch::PunchStart)
...
int oCard::getFinishPunchCode() const {
  ...
  if (it != punches.rend()) {
    if (it->getTypeCode() == SpecialPunch::PunchFinish)
      return it->getPunchUnit();
  }
  return 0;
}
