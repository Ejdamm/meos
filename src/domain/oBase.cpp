...
void oBase::setExtIdentifier(long long id)
{
  getDI().setInt64("ExtId", id);
}

long long oBase::getExtIdentifier() const
{
  return getDCI().getInt64("ExtId");
}

wstring oBase::getExtIdentifierString() const {
  long long raw = getExtIdentifier();
  wchar_t res[16];
  if (raw == 0)
    return L"";
...
void oBase::converExtIdentifierString(long long raw, wchar_t bf[16])  {
  if (raw & BaseGenStringFlag)
...
long long oBase::converExtIdentifierString(const wstring &str) {
  long long val;
...
void oBase::setExtIdentifier(const wstring &str) {
  long long val = converExtIdentifierString(str);
  setExtIdentifier(val);
}

int oBase::idFromExtId(long long val) {
  int basePart = int(val & 0x0FFFFFFF);
  if (basePart == val)
    return basePart;

  long long hash = (val&ExtStringMask) % 2000000011ul;
