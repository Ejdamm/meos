...
struct GeneralResultCtr {
  ...
  bool operator<(const GeneralResultCtr& other) const { return name < other.name; }
  bool isDynamic() const { return ptr && ptr->isDynamic(); }
  wstring getName() const { return ptr ? ptr->getName() : name; }
};
...
protected:
  ...
  mutable pair<int, map<pair<SpecialPunch, int>, int>> typeUnitPunchTimeAdjustment;

  /** Return start/finish/check units existing in punches/cards*/
  void getExistingUnits(vector<pair<SpecialPunch, int>>& typeUnit);
  ...
  void generateStatisticsPart(gdioutput &gdi, const vector<ClassType> &type,
...
public:
  ...
  int getUnitAdjustment(SpecialPunch type, int unit) const;
...
public:
  ...
  void setNameMode(NameMode newNameMode) { currentNameMode = newNameMode; }
  ...
  struct ResultEvent {
    ...
    ResultEvent(pRunner r, int time, int control, RunnerStatus status):
        ...
        resultScore(0), place(-1), runTime(0), partialCount(0), legNumber(short(r->getLeg())) {}
...
public:
  ...
  void convertTimes(pRunner runner, void *sic) const;

  pCard getCard(int Id) const;
  pCard getCardByNumber(int cno) const;
  bool isCardRead(const void *card) const;
  ...
  /** Try to find the class that best matches the card.
      ...
      Zero = exact match */
  int findBestClass(const void *card, vector<pClass> &classes) const;
...
public:
  ...
  map<int, SpecialPunch> getPunchMapping() const;
  void definePunchMapping(int code, SpecialPunch value);
  ...
  void generateTestCard(void *sic) const;
