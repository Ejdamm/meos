...
SportIdent::SportIdent(HWND hWnd, DWORD Id, bool readVoltage) : readVoltage(readVoltage) {
  ...
  //punchMap[SpecialPunch::PunchStart] = SpecialPunch::PunchStart;
  punchMap[SpecialPunch::PunchCheck] = SpecialPunch::PunchCheck;
  punchMap[SpecialPunch::PunchFinish] = SpecialPunch::PunchFinish;
}
...
#ifdef DEBUG_SI
              char str[128];
              snprintf(str, "EXTENDED: Card = %d, Station = %d, StationMode = %d", Card, Station, si.StationMode);
...
#ifdef DEBUG_SI
              char str[128];
              snprintf(str, "OLD: Card = %d, Station = %d, StationMode = %d", DCard, Station, si.StationMode);
              MessageBox(NULL, str, NULL, MB_OK);
#endif
...
void SportIdent::addPunch(DWORD Time, int Station, int Card, int Mode) {
  ...
  auto mapPunch = [this](int code) {
    ...
    else if (code == SpecialPunch::PunchStart)
      return int(SpecialPunch::PunchCheck); // Do not allow start unless explicitly set
    else
      return code;
  };
...
map<int, SpecialPunch> SportIdent::getSpecialMappings() const {
  map<int, SpecialPunch> res;
  for (int j = 1; j < punchMap.size(); j++) {
    if (punchMap[j] > 0)
      res[j] = SpecialPunch(punchMap[j]);
  }
  return res;
}
...
void SportIdent::addSpecialMapping(int code, SpecialPunch p) {
