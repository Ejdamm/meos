...
static volatile int mysqlConnecting=0;
static volatile int mysqlStatus=0;

void initMySQLCriticalSection(bool /*init*/) {
  // No-op: std::mutex does not require explicit initialization or destruction
}
...
void reconnectThread(oEvent *oe) {
  {
    std::lock_guard<std::mutex> lock(CS_MySQL);
    mysqlConnecting=1;
    mysqlStatus=0;
...
void MySQLReconnect::status(gdioutput &gdi) {
  ...
  if (interval>0){
    ...
    gdi.addTimer(gdi.getCY(),  gdi.getCX()+10, timerCanBeNegative, int((getTickMs64()-timeout)/1000));
  }
  else {
